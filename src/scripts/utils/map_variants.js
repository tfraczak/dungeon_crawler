...
// (`src/scripts/world/room.js`) use to look up `gameState.bgImgs`. The value
// is the number of map background variants that exist on disk for that
// configuration -- i.e. the folder
// `src/assets/images/map_imgs/<numExits>/<dirs>/` must contain
// `map0.png .. map<count - 1>.png`.
//
// To add a new variant for a configuration, drop the new file in as the
// next index (`map<count>.png`) and bump the count here. Configs not listed
