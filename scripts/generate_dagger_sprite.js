/*
 * Procedurally generates a dagger sprite.
 *
 * Output: src/assets/items/equipment/weapons/daggers/dagger/sprite.png
 *         dist/assets/items/equipment/weapons/daggers/dagger/sprite.png
 *         (48 x 48, single frame)
 *
 * Zero-dependency: uses Node's built-in `zlib` + `Buffer` to hand-encode the
 * PNG. Run with: node scripts/generate_dagger_sprite.js
 */
...
function buildFrame() {
  ...
  // Short triangular blade, angled lower-left to upper-right.
  drawLine(out, 16, 32, 27, 21, 5, "O");
  drawLine(out, 17, 31, 26, 22, 3, "s");
  drawLine(out, 18, 30, 25, 23, 2, "m");
  drawLine(out, 20, 28, 25, 23, 1, "h");
  ...
  // Small guard.
  drawLine(out, 14, 30, 18, 34, 3, "G");
  drawLine(out, 15, 30, 18, 33, 1, "g");

  // Compact grip and pommel.
  drawLine(out, 11, 38, 15, 34, 3, "L");
  drawLine(out, 12, 37, 15, 34, 1, "l");
  setPx(out, 13, 36, "H");
  drawDisc(out, 9, 40, 1, "p");
  setPx(out, 9, 40, "P");
